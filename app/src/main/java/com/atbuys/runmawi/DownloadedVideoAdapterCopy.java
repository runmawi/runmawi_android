...
public class DownloadedVideoAdapterCopy extends RecyclerView.Adapter<DownloadedVideoAdapterCopy.MyViewHolder> { //implements Filterable

    List<Download> videosList;
    //    private ValueFilter mFilter = new ValueFilter();
    Context context;
    Context downloadActivity;
...
//            imgDownloadPlayPause = view.findViewById(R.id.img_download_play_pause);
    ...
    public DownloadedVideoAdapterCopy(Context context, Context downloadActivity) {
            ...
            for (String key : o.keySet()) {
                if (key.equals("percentDownloaded")) {
                    ...
                    if (!videoModel.getVideoName().isEmpty()) {
                       ...
                       Picasso.get().load(videoModel.getVideoId()).fit().into(holder.imageView);
                    }
...
//                    String downloadInMb = AppUtil.getProgressDisplayLine(download.getBytesDownloaded(), downloadRequest.data.length);
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if (download.state == Download.STATE_COMPLETED) {
            ...
            holder.imgMenuOverFlow.setImageResource(R.drawable.downloaded);
        }

        if (download.state == Download.STATE_DOWNLOADING) {

            holder.imgMenuOverFlow.setImageResource(R.drawable.loading_light_download);
        }
        if (download.state == Download.STATE_STOPPED) {

            holder.imgMenuOverFlow.setImageResource(R.drawable.download_pause_light);
        }
