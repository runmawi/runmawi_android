...
class chennalparnerAdapter extends RecyclerView.Adapter<chennalparnerAdapter.MyViewHolder> {



    private ArrayList<channels> thismaylikelilst;
    private Context context;

    private static int currentPosition = 0;

    public chennalparnerAdapter(ArrayList<channels> thismaylikelilst, Context context) {
        this.thismaylikelilst = thismaylikelilst;
        this.context = context;
    }

    public chennalparnerAdapter(ArrayList<channels> channelvideosdata, NotificationsFragment notificationsFragment) {
    }
    ...
    public chennalparnerAdapter(ArrayList<channels> thismaylikelilst) {
        this.thismaylikelilst = thismaylikelilst;
    }
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
      ...
      //  holder.id.setText(thismaylikelilst.get(position).getId());
       ...
       // holder.name.setText(thismaylikelilst.get(position).getTitle());
        ...
        Picasso.get().
                load( thismaylikelilst.get(position).getChannel_image())
                .into(holder.image);
