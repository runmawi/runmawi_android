...
class contentparnerAdapter extends RecyclerView.Adapter<contentparnerAdapter.MyViewHolder> {



    private ArrayList<ContentPartner> thismaylikelilst;
    private Context context;

    private static int currentPosition = 0;

    public contentparnerAdapter(ArrayList<ContentPartner> thismaylikelilst, Context context) {
        this.thismaylikelilst = thismaylikelilst;
        this.context = context;
    }

    public contentparnerAdapter(ArrayList<ContentPartner> channelvideosdata, NotificationsFragment notificationsFragment) {
    }
    ...
    public contentparnerAdapter(ArrayList<ContentPartner> thismaylikelilst) {
        this.thismaylikelilst = thismaylikelilst;
    }
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
      ...
      //  holder.id.setText(thismaylikelilst.get(position).getId());
       ...
       // holder.name.setText(thismaylikelilst.get(position).getTitle());
        ...
        Picasso.get().
                load( thismaylikelilst.get(position).getPicture())
                .into(holder.image);
